...
alter table components add column if not exists manufacturer text;
...
create index if not exists idx_components_purchased_date on components(purchased_date);

-- 3. Transactions table (run only if you don't have it yet)
